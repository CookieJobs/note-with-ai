...
export default function AdminShell({ children }: { children: React.ReactNode }) { const path = usePathname(); const router = useRouter(); const [admin, setAdmin] = useState<AdminIdentity | null>(null); const [loading, setLoading] = useState(true); const [forbidden, setForbidden] = useState(false);
  useEffect(() => { if (path === '/admin/login') { setLoading(false); return; } adminFetch<{ admin: AdminIdentity }>('/api/admin/auth/me').then((r) => setAdmin(r.admin)).catch((error) => { if (error?.status === 403) setForbidden(true); }).finally(() => setLoading(false)); }, [path]);
  if (path === '/admin/login') return <>{children}</>; if (loading) return <main className={styles.center}>正在验证管理员会话…</main>; if (forbidden) return <main className={styles.center}><p role="alert">当前管理员角色无权访问此页面</p></main>; if (!admin) return <main className={styles.center}>会话已失效，请重新登录</main>;
