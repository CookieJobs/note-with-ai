...
export default function ReasonDialog({ onSubmit, onClose }: Props) {
  ...
  async function submit() { setPending(true); setError(''); try { await onSubmit(reason.trim()); } catch (e: unknown) { setError(e instanceof Error ? e.message : '提交失败'); } finally { setPending(false); } }
