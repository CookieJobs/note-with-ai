...
test('feedback model has strict bounded public fields and workflow states', async () => {
  ...
  assert.deepEqual((schema.path('status') as any).enumValues, ['new', 'in_progress', 'resolved', 'closed']);
...
test('feedback admin DTO never includes internal note in the public response', async () => {
  ...
  const result = toPublicFeedback({ _id: 'f1', status: 'new', category: 'bug', content: 'hello', internalNote: 'private', createdAt: new Date() });
  assert.deepEqual(result, { id: 'f1', status: 'new', category: 'bug', content: 'hello', createdAt: result.createdAt });
