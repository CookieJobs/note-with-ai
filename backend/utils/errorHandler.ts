...
export class ErrorHandler {
  ...
  // 创建数据库错误
  static createDatabaseError(message: string, originalError?: unknown): AppError {
    return new AppError(
      message,
      ErrorType.DATABASE,
      500,
  ...
  // 创建外部API错误
  static createExternalApiError(message: string, service: string, originalError?: unknown): AppError {
    return new AppError(
      message,
  ...
  static createPayloadTooLargeError(message: string = '请求体过大，请压缩图片或减少内容后重试', originalError?: unknown): AppError {
    return new AppError(
      message,
  ...
  // 创建内部服务器错误
  static createInternalError(message: string = '内部服务器错误', originalError?: unknown): AppError {
    return new AppError(
      message,
      ErrorType.INTERNAL,
      500,
  ...
  static async wrapAsync<T>(
    ...
    try {
      ...
      logger.error(`❌ ${operationName} 失败:`, (error as Error).message || error);
      ...
      if ((error as Error).name === 'MongoError' || (error as Error).name === 'MongooseError') {
