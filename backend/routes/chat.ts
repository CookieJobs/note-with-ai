...
import { chatWithDeepSeek, chatWithDeepSeekStream, summarizeChatTitle } from '../services/deepseek';
...
router.post('/', authenticateToken, asyncHandler(async (req: any, res: any) => {
  ...
  try {
    let fullReply = '';
    const stream = await chatWithDeepSeekStream(cleanedMessages);

    // 只有在获取到流之后才设置 SSE 响应头
    res.setHeader('Content-Type', 'text/event-stream');
    res.setHeader('Cache-Control', 'no-cache, no-transform'); // no-transform 防止代理压缩导致无法流式传输
    res.setHeader('Connection', 'keep-alive');
    res.setHeader('X-Accel-Buffering', 'no'); // 禁用 Nginx 等反向代理的缓冲
    res.flushHeaders(); // 立即将 header 冲刷下去，配合上方 headers 确保长连接真正流式
    ...
    const finalMessages = [...cleanedMessages, { role: 'assistant' as const, content: fullReply }];
    ...
    // 如果响应还没开始发送（headersSent 为 false），则可以发送标准的 JSON 错误
    if (!res.headersSent) {
