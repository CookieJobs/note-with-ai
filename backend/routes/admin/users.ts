...
router.get('/', asyncHandler(async (req, res) => {
  ...
  validateAdminDateRange(raw.createdFrom, raw.createdTo);
  const data = await listUsers({ query: typeof raw.query === 'string' ? raw.query : undefined, status: raw.status === 'active' || raw.status === 'disabled' ? raw.status : undefined, from: typeof raw.createdFrom === 'string' ? raw.createdFrom : undefined, to: typeof raw.createdTo === 'string' ? raw.createdTo : undefined, page, limit, role: req.admin!.role });
...
router.post('/:id/status', requireAdminPermission('users:status'), requireAdminMutationOrigin, asyncHandler(async (req, res) => {
  ...
  const result = await runAuditedAdminCommand({ actorId: req.admin!.id, requestId: req.requestId, action: 'user.status_changed', targetType: 'User', targetId: req.params.id, metadata: { reason: reason.trim(), nextStatus: isActive ? 'active' : 'disabled' } }, () => setUserActive(req.params.id, isActive));
