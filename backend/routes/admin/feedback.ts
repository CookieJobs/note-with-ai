...
router.patch('/:id', requireAdminPermission('feedback:write'), requireAdminMutationOrigin, asyncHandler(async (req, res) => {
  const body = req.body ?? {}; const allowed = ['status', 'internalNote', 'assignedToSelf', 'reason']; if (!Object.keys(body).some((key) => key !== 'reason' && allowed.includes(key)) || Object.keys(body).some(key => !allowed.includes(key))) throw ErrorHandler.createValidationError('反馈更新字段无效');
  if (typeof body.reason !== 'string' || body.reason.trim().length < 5 || body.reason.trim().length > 200) throw ErrorHandler.createValidationError('反馈更新原因需为 5-200 个字符');
  ...
  const changedFields = Object.keys(body).filter((key) => key !== 'reason').join(',');
  const result = await runAuditedAdminCommand({ actorId: req.admin!.id, requestId: req.requestId, action: 'feedback.updated', targetType: 'UserFeedback', targetId: req.params.id, metadata: { reason: body.reason.trim(), changedFields } }, () => updateFeedback(req.params.id, { status: body.status, internalNote: body.internalNote, assignedToSelf: body.assignedToSelf }, req.admin!.id, req.admin!.role));
