...
import { FEEDBACK_CATEGORIES } from '../models/UserFeedback';

const router = express.Router();
router.post('/', authenticateToken, asyncHandler(async (req, res) => {
  const { content, category, contact, appVersion } = req.body ?? {};
  if (typeof content !== 'string' || content.trim().length < 5 || content.trim().length > 2000 || !FEEDBACK_CATEGORIES.includes(category) || (contact !== undefined && (typeof contact !== 'string' || contact.length > 200)) || (appVersion !== undefined && (typeof appVersion !== 'string' || appVersion.length > 50))) throw ErrorHandler.createValidationError('反馈内容无效');
