...
export async function listFeedback(input: { status?: typeof FEEDBACK_STATUSES[number]; category?: typeof FEEDBACK_CATEGORIES[number]; userId?: string; assignedToSelf?: boolean; assignedTo?: string; page?: number; limit?: number; from?: string; to?: string }) {
  ...
  const filter: any = {}; if (input.status) filter.status = input.status; if (input.category) filter.category = input.category; if (input.userId) filter.userId = input.userId; if (input.assignedTo) filter.assignedTo = input.assignedTo;
