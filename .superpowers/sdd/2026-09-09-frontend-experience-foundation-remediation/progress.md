...
- Task 14 manual status: actual macOS system-dark appearance and VoiceOver audio require explicit OS-setting authorization. Equivalent 320/390 reflow, automated dark/reduced-motion contracts, and live accessibility-tree keyboard paths passed.
...
- Task 14 final root gates: frontend 41 files / 302 tests, lint, typecheck, axe, production build and `/notes` 231.0 kB budget pass; backend 169 tests, typecheck and build pass; `git diff --check d09b8c0..HEAD` pass. Only the two explicitly authorized OS-state checks remain unexecuted.
