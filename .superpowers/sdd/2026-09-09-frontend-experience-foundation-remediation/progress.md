...
- Task 14: ruling authorized the smallest upstream Auth/Note ARIA corrections and a scoped Notes code-splitting remediation after axe found invalid panel/menu ARIA and the production bundle measured 470 kB. Cost: two upstream components/tests plus dynamic viewer/schema boundary; no threshold was weakened. Complete pending focused acceptance commit: axe gate green, `/notes` 231 kB First Load JS (45.1% reduction), frontend 263 tests, backend 168 tests, builds/typechecks/lint/diff check green. Live seeded-account, three-theme, dark-mode, zoom, and VoiceOver visual matrix remains manual-only evidence.
