...
Root supplied the 1000-note live measurements above. The latest 320px root measurement found Profile `编辑资料` and `修改密码` at 39px wide × 94.39px high, with computed `min-width: auto` and padding `7.2px 12px`; the scoped CSS/style-contract remediation now requires `min-width: 2.75rem` for those two controls. The post-fix 320/390/768/1440 rerun, including the Profile remeasure, 251-note account, browser-driven theme toggle, and VoiceOver session, remain **pending root verification**. Consequently the full route-by-route matrix and three rendered Profile-atmosphere screenshots are still open manual P1 acceptance requirements, not waived automated failures. Automated gates did not report a P0/P1, but no final zero-P1 ruling can be made until the root verification matrix is completed.
