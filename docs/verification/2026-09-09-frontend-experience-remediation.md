...
The in-app browser did not inherit macOS dark appearance, so root repeated the production check in Chrome with `prefers-color-scheme: dark` confirmed true. That live check exposed a selector-specificity defect missed by the original source contract: the system-dark tokens were declared through zero-specificity `:where(...)` and lost to the base `:root` declarations. Commit `b943cff` adds a RED-to-GREEN regression and changes the selector to `:root:not(.light)`. After rebuilding, Chrome computed the intended dark semantic tokens (`#101522` page, `#191f2e` raised surface, `#f5f7fb` primary text, `#c3cad8` secondary text, `#3d485e` border, and `#aebfff` primary accent); the Auth compact card was visibly dark and had no horizontal overflow. The 320/390 responsive passes provide the layout-equivalent narrow CSS viewport evidence for 200% reflow.

With explicit user authorization, root also enabled macOS VoiceOver and exercised the live Auth flow in Chrome. ArrowRight moved the selected/focused tab from Login to Register, Tab reached the named email field, and the password visibility control exposed `显示密码`, toggled to `隐藏密码`, and reported the changed state. VoiceOver was then restored to off. This supplements the automated axe, accessibility-tree, focus-restoration, contrast, and reduced-motion evidence.

## Impeccable acceptance review

| Dimension | Score | Evidence |
| --- | ---: | --- |
| Accessibility | Pass | Axe core states, dialog lifecycle tests, semantic contrast tests, live accessibility-tree/keyboard paths, focus restoration, named controls, and an actual macOS VoiceOver Auth path pass. |
...
| Theming | Live pass | Three live Profile atmosphere fixtures pass, and Chrome under actual macOS dark appearance computes and visibly renders the corrected dark semantic palette. |
...
| **Final acceptance** | **Pass** | Code, automated gates, live responsive/long-list/theme-fixture checks, actual macOS dark appearance, and actual VoiceOver keyboard/semantic checks pass. |
...
- Final system-dark cascade closure: `b943cff650f3ccc4e8053a86f8613799db9eb8e8`
