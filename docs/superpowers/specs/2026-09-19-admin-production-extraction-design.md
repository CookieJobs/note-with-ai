...
2. 后台路由与页面：`/admin/login`、概览、用户、AI、反馈、系统和审计；API 位于 `/api/admin/*`。其中必须纳入经用户验收的 `codex/admin-ui-refresh` UI 优化：统一的后台外壳和浅色工作台样式、概览图表和指标层级、各运营页面的表格/筛选/空状态/原因弹窗、会话超时恢复，以及概览和 AI 时间范围切换的请求竞态保护。
...
基础管理 UI 恢复并通过测试后，再从 `codex/admin-ui-refresh` 的单一已验证提交中只摘取下列路径：`frontend/src/app/admin/**` 和 `docs/verification/2026-09-19-admin-ui-refresh.md`。该补丁的来源基线是 `50956a1`，但其自身差异不得包含任何其他路径；移植前后均执行路径审查与前端完整测试。
